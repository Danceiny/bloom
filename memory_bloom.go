...
// A MemoryBloomFilter is a representation of a set of _n_ items, where the main
// requirement is to make membership queries; _i.e._, whether an item is a
// member of a set.
...
func NewMemoryBloomFilter(m uint, k uint) *MemoryBloomFilter {
	return &MemoryBloomFilter{max(1, m), max(1, k), bitset.New(m)}
}

// From creates a new Bloom filter with len(_data_) * 64 bits and _k_ hashing
// functions. The data slice is not going to be reset.
func From(data []uint64, k uint) *MemoryBloomFilter {
	m := uint(len(data) * 64)
	return &MemoryBloomFilter{m, k, bitset.From(data)}
}
...
// false positive rate
func NewWithEstimates(n uint, fp float64) *MemoryBloomFilter {
	m, k := EstimateParameters(n, fp)
	return NewMemoryBloomFilter(m, k)
}

// Cap returns the capacity, _m_, of a Bloom filter
func (f *MemoryBloomFilter) Cap() uint {
	return f.m
}

// K returns the number of hash functions used in the MemoryBloomFilter
func (f *MemoryBloomFilter) K() uint {
	return f.k
}
...
func (f *MemoryBloomFilter) Merge(g *MemoryBloomFilter) error {
...
func (f *MemoryBloomFilter) Copy() *MemoryBloomFilter {
	fc := NewMemoryBloomFilter(f.m, f.k)
	_ = fc.Merge(f) // #nosec
	return fc
}

// AddString to the Bloom Filter. Returns the filter (allows chaining)
func (f *MemoryBloomFilter) AddString(data string) BF {
...
// TestString returns true if the string is in the MemoryBloomFilter, false otherwise.
// If true, the result might be a false positive. If false, the data
// is definitely not in the set.
func (f *MemoryBloomFilter) TestString(data string) bool {
	return f.Test([]byte(data))
}

// TestLocations returns true if all locations are set in the MemoryBloomFilter, false
// otherwise.
func (f *MemoryBloomFilter) TestLocations(locs []uint64) bool {
...
func (f *MemoryBloomFilter) TestAndAdd(data []byte) bool {
	present := true
	h := baseHashes(data)
	for i := uint(0); i < f.k; i++ {
		l := f.Location(h, i)
...
// Returns the result of Test.
func (f *MemoryBloomFilter) TestAndAddString(data string) bool {
	return f.TestAndAdd([]byte(data))
}

// ClearAll clears all the data in a Bloom filter, removing all keys
func (f *MemoryBloomFilter) ClearAll() *MemoryBloomFilter {
	f.b.ClearAll()
	return f
}

// EstimateFalsePositiveRate returns, for a MemoryBloomFilter with a estimate of m bits
// and k hash functions, what the false positive rate will be
// while storing n entries; runs 100,000 tests. This is an empirical
// test using integers as keys. As a side-effect, it clears the MemoryBloomFilter.
func (f *MemoryBloomFilter) EstimateFalsePositiveRate(n uint) (fpRate float64) {
...
// bloomFilterJSON is an unexported type for marshaling/unmarshaling MemoryBloomFilter struct.
...
func (f *MemoryBloomFilter) MarshalJSON() ([]byte, error) {
	return json.Marshal(bloomFilterJSON{f.m, f.k, f.b})
}

// UnmarshalJSON implements json.Unmarshaler interface.
func (f *MemoryBloomFilter) UnmarshalJSON(data []byte) error {
...
// WriteTo writes a binary representation of the MemoryBloomFilter to an i/o stream.
// It returns the number of bytes written.
func (f *MemoryBloomFilter) WriteTo(stream io.Writer) (int64, error) {
...
// ReadFrom reads a binary representation of the MemoryBloomFilter (such as might
// have been written by WriteTo()) from an i/o stream. It returns the number
// of bytes read.
func (f *MemoryBloomFilter) ReadFrom(stream io.Reader) (int64, error) {
	var m, k uint64
...
func (f *MemoryBloomFilter) GobEncode() ([]byte, error) {
...
func (f *MemoryBloomFilter) GobDecode(data []byte) error {
...
func (f *MemoryBloomFilter) Equal(g *MemoryBloomFilter) bool {
