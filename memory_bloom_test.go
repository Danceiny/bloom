...
func TestEqual(t *testing.T) {
	f := NewMemoryBloomFilter(1000, 4)
	f1 := NewMemoryBloomFilter(1000, 4)
	g := NewMemoryBloomFilter(1000, 20)
	h := NewMemoryBloomFilter(10, 20)
	n1 := []byte("Bess")
	f1.Add(n1)
	if !f.Equal(f) {
...
func TestMerge(t *testing.T) {
	...
	g := NewMemoryBloomFilter(1000, 4)
	n2 := []byte("g")
	g.Add(n2)

	h := NewMemoryBloomFilter(999, 4)
	n3 := []byte("h")
	h.Add(n3)

	j := NewMemoryBloomFilter(1000, 5)
	n4 := []byte("j")
	j.Add(n4)
...
func TestCopy(t *testing.T) {
	...
	// copy here instead of NewMemoryBloomFilter
	g := f.Copy()
	n2 := []byte("g")
	g.Add(n2)
